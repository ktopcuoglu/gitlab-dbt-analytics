...
final_sql_query_dict = main(test_cases)
...
for sql_metric, sql_query in final_sql_query_dict.items():
    ...
    assert sql_query == results[sql_metric]
...
for i, test_case in enumerate(test_cases):
    assert find_keyword_index(test_case, "SELECT") == results[i]

results = [2, 0, 4, 10]
# 2. looking for FROM keyword
for i, test_case in enumerate(test_cases):
    assert find_keyword_index(test_case, "FROM") == results[i]
...
results = ["SELECT 1", "SELECT abc from def", "SELECT (SELECT 1)"]

for i, test_case in enumerate(test_cases):
    assert prepare_sql_statement(test_case) == results[i]
