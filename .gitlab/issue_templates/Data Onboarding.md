...
- In case if you encounter any error then please run the command `open/Applications/Docker.app` to ensure that docker is up and running. You can also run the command `docker run hello-world` to confirm the same. You should see a `Hello` from Docker message which confirms that the docker installation is working correctly.
...
- [ ] Run `dbt compile` from within the container to know that your connection has been successful, you are in the correct location, and everything will run smoothly.  If you see an error like `Schema 'ANALYTICS.NAME_SCRATCH_STAGING' does not exist or not authorized`, it is because that schema hasn't been created by dbt in Snowflake yet. Since `dbt compile` doesn't actually create anything in the database, it won't create it for you.  To fix this, feel free to run a small model with dbt `dbt run --models sfdc` and then `dbt compile` should work as long as there weren't any issues with running the model. Please note that `sfdc` would actually not select any models and this can be confirmed by checking [here](https://dbt.gitlabdata.com/#!/model/model.gitlab_snowflake.greenhouse_application_stages_source?g_v=1&g_i=sfdc). For more details on the syntax for how to select and run the models, please refer to this [page](https://docs.getdbt.com/reference/node-selection/syntax#examples). It is also recommended to run `dbt run --models +staging.sfdc` which will pull all the models in the `/models/staging/sfdc/` directory as well as their parents.
- [ ] Run `Exit` command to come out of the dbt docker and test the command `make help` and use it to understand how to use `make dbt-docs` and access it from your local machine.
