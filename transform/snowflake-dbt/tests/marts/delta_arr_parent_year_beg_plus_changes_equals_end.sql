...
WITH test AS (
    ...
    FROM {{ ref('mart_delta_arr_parent_year') }}
    GROUP BY 1,2
    ...
    SELECT
      ...
      CASE
        WHEN ROUND(test_two.beg_arr - LAG(test_two.end_arr) OVER (PARTITION BY test_two.ultimate_parent_account_id ORDER BY test_two.arr_year)) IS NULL
        THEN 0
        ELSE ROUND(test_two.beg_arr - LAG(test_two.end_arr) OVER (PARTITION BY test_two.ultimate_parent_account_id ORDER BY test_two.arr_year))
      END                                                                                    AS beg_end_variance
    FROM test
    LEFT JOIN arr_data_mart
      ON test.arr_year = arr_data_mart.arr_month
      AND test.ultimate_parent_account_id = arr_data_mart.ultimate_parent_account_id
    LEFT JOIN test_two
      ON test.arr_year = test_two.arr_year
