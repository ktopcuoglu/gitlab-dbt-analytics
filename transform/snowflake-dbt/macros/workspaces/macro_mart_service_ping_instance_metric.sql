{% macro macro_mart_service_ping_instance_metric(model_name1) %}
  ...
  SELECT
    * FROM {{ ref(model_name1) }}
    ...
    SELECT
      ...
      -- Primary Key
      {{ dbt_utils.surrogate_key(['dim_service_ping_instance_id', 'metrics_path']) }} AS service_ping_instance_metric_id,
