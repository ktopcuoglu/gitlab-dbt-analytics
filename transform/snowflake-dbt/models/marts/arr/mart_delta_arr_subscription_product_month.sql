...
, mart_arr AS (
    ...
    SELECT
      ...
      COALESCE(LAG(quantity) OVER (PARTITION BY dim_subscription_id, product_tier_name ORDER BY arr_month),0) AS previous_quantity,
      COALESCE(LAG(arr) OVER (PARTITION BY dim_subscription_id, product_tier_name ORDER BY arr_month),0) AS previous_arr,
      ROW_NUMBER() OVER (PARTITION BY dim_subscription_id, product_tier_name ORDER BY arr_month) AS row_number
    ...
    SELECT
      {{ dbt_utils.surrogate_key(['type_of_arr_change.arr_month', 'type_of_arr_change.dim_subscription_id','type_of_arr_change.product_tier_name']) }}
                                                                    AS primary_key,
