WITH source AS (
  ...
  SELECT
    ...
    REPLACE(invoice_amount, '(', '')                    AS transformation_1,
    REPLACE(transformation_1, ')', '')                  AS transformation_2,
    REPLACE(transformation_2, ',', '')                  AS invoice_amount,
    due_date::DATE                                      AS due_date,
    terms::VARCHAR                                      AS terms,
    days_aging::NUMBER                                  AS days_aging,
    aging_bucket::VARCHAR                               AS aging_bucket,
    REPLACE(invoice_balance, '(', '')                   AS transformation_3,
    REPLACE(transformation_3, ')', '')                  AS transformation_4,
    REPLACE(transformation_4, ',', '')                  AS invoice_balance,
