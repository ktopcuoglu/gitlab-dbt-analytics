...
WITH account_prep AS (
    ...
    LEFT JOIN geo_region         
      ON account_prep.dim_geo_region_name = geo_region.dim_geo_region_name
    LEFT JOIN geo_sub_region
      ON account_prep.dim_geo_sub_region_name = geo_sub_region.dim_geo_sub_region_name
    LEFT JOIN geo_area      
      ON account_prep.dim_geo_area_name = geo_area.dim_geo_area_name
    LEFT JOIN sales_segment
      ON account_prep.dim_sales_segment_name = sales_segment.dim_sales_segment_name
    LEFT JOIN sales_territory
      ON account_prep.dim_sales_territory_name = sales_territory.dim_sales_territory_name
    LEFT JOIN industry      
      ON account_prep.dim_industry_name = industry.dim_industry_name
)
