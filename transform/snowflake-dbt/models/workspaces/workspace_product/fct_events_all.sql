WITH dim_subscription AS (
  ...
  WHERE SUBSTR(value, 1, 1) != '{' and IS_REAL(TO_VARIANT(value)) = true
    ...
    FROM flattened_usage
        JOIN
    dim_usage_ping_metric AS metric
  ...
  dim_subscription
    ...
    SELECT top 100 event_primary_key,
            ...
            CASE
                WHEN gitlab_dotcom_usage_data_events.stage_name IS NULL THEN gitlab_dotcom_xmau_metrics.stage_name
                ELSE gitlab_dotcom_usage_data_events.stage_name
               end AS stage_name,
            group_name,
            section_name,
            smau,
            gmau,
            is_umau AS umau
        FROM gitlab_dotcom_usage_data_events
            JOIN gitlab_dotcom_xmau_metrics
        ON gitlab_dotcom_usage_data_events.event_name = gitlab_dotcom_xmau_metrics.event_name
), dim_license AS (
...
SELECT *
    ...
    FROM bdg_namespace_order_subscription -- this appears to be where at least some duplication is happening, how do I join here to not duplicate from bdg
