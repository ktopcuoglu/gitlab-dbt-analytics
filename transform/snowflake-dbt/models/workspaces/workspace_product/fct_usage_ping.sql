...
{%- set settings_columns = dbt_utils.get_column_values(table=ref('prep_usage_ping_cte_metrics_setting'), column='metrics_path', max_records=1000, default=['']) %}
...
{{ simple_cte([
    ...
    ('raw_usage_data', 'version_raw_usage_data_source'),
    ('prep_usage_ping_cte_metrics_setting', 'prep_usage_ping_cte_metrics_setting'),
    ('dim_date', 'dim_date')
    ])
...
, source AS (
  ...
  WHERE product_delivery_type = 'Self-Managed'
    ...
    SELECT
      {{ dbt_utils.star(from=prep_usage_ping_cte, relation_alias='prep_usage_ping_cte', except=['EDITION', 'CREATED_AT', 'SOURCE_IP']) }}, /* Might need to look into this line, was referencing the model but now want it to reference CTE, not sure if it can within macro*/
