WITH dim_billing_account AS (
  ...
  WHERE subscription_status NOT IN ('Draft', 'Expired')
   ...
   INNER JOIN zuora_subscription_spined sub_2
     ...
     AND DATE_TRUNC('month',sub_2.subscription_end_date) >= '2023-02-01'
   ...
   SELECT
     ...
     CASE
       WHEN is_paid_in_full = FALSE AND fct_charge.effective_end_month <= '2023-01-01' AND is_myb = TRUE THEN months_of_future_billings * fct_charge.mrr
       ELSE 0
     END                                                                                  AS estimated_fy23_future_billings,
   ...
   WHERE fct_charge.effective_start_month <= '2022-01-01'
  ...
  {{ dbt_utils.group_by(n=18) }}
   ...
   FROM agg_charge_term_greater_than_12 WHERE charge_term BETWEEN 13 AND 24 AND effective_end_month > '2023-01-01' {{ dbt_utils.group_by(n=18) }}
   ...
   FROM agg_charge_term_greater_than_12 WHERE charge_term BETWEEN 25 AND 36 AND effective_end_month > '2023-01-01'  {{ dbt_utils.group_by(n=18) }}
   UNION ALL
   ...
   FROM agg_charge_term_greater_than_12 WHERE charge_term BETWEEN 37 AND 48 AND effective_end_month > '2023-01-01'  {{ dbt_utils.group_by(n=18) }}
   UNION ALL
   ...
   FROM agg_charge_term_greater_than_12 WHERE charge_term BETWEEN 49 AND 60 AND effective_end_month > '2023-01-01'  {{ dbt_utils.group_by(n=18) }}
   UNION ALL
   ...
   SELECT * FROM sixty_mth_term
    ...
    SELECT
      ...
      CASE
        WHEN base.subscription_end_month BETWEEN '2022-02-01' AND '2023-01-01' AND base.is_myb_with_multi_subs = FALSE THEN TRUE
        ELSE FALSE
      END                            AS is_atr,
      base.is_myb,
      ...
      base.estimated_fy23_future_billings
    FROM combined
    LEFT JOIN dim_date
      ON combined.effective_end_month = dim_date.first_day_of_month
    LEFT JOIN base
      ON combined.charge_id = base.charge_id
    WHERE combined.effective_end_month BETWEEN '2022-02-01' AND '2023-01-01'
      AND day_of_month = 1
