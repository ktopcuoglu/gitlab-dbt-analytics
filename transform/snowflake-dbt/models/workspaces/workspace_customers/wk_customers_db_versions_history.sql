...
SELECT customers_db_versions.item_id AS order_from_source, old_changes.*, COALESCE(LAG(customers_db_versions.created_at) OVER (PARTITION BY old_changes.order_id ORDER BY customers_db_versions.created_at ASC), old_changes.created_at) AS updated_at
...
order_end_date,
order_quantity,
actual_object_creation_date AS order_created_at,
dim_namespace_id,
dim_namespace_name,
amendment_type,
order_is_trial,
