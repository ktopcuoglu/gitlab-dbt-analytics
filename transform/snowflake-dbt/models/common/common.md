...
**Description:** Atomic Level Service Ping Instance Level Data
- Sums, Counts and Percents of Usage Data (called metrics) along wth the Instance Server and Implementation Configuration data collected from each Instance of GitLab Implementations.  Implementations can be Customer Hosted (Self-Managed) or GitLab Hosted (referred to as SaaS or Dotcom data).  The data is collected from each Instance of a GitLab implementation at a particular point in time and sent to GitLab Corporate.  This is normally done on a weekly basis.  The Instance Owner determines whether to send this data and how much to send through the application configuration. 
- Atomic (lowest grain) at the Instance Level  
...
**Filters:**
- UUID is NOT NULL
- version NOT LIKE '%VERSION%'

**Business Logic in this Model:** 
- `data_source` = 'VERSION_DB'
  - This model brings in data from Self-Managed Implementations which goes through the Versions Application and Database  

**Other Comments:**
- The `fct_ping_instance` table is built directly from the [prep_ping_instance table](https://gitlab-data.gitlab.io/analytics/#!/model/model.gitlab_snowflake.prep_ping_instance) which brings in All Service Ping data one record per Service Ping to include a 'Payload' column with all Metrics that can be 2000+ in number. 
- Service Ping data (configuration and metrics) is captured at a particular point in time for a specific grain.  The metrics and the metrics are calculated for different time frames. for a specific time-frame in reference to the time the Ping is generated.  Therefore, this grain can not be broken down or aggregated successfully. 
