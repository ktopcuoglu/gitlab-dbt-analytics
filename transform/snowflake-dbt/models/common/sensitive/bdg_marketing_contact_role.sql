WITH gitlab_namespaces AS (
    ...
    SELECT *
    FROM {{ref('prep.gitlab_dotcom.gitlab_dotcom_namespaces_source') }}

), gitlab_members AS (
    SELECT *
    FROM {{ref('prep.gitlab_dotcom.gitlab_dotcom_members_source') }}
    ...
    SELECT *
    FROM {{ref('prep.zuora.zuora_subscription_source') }}
    ...
    FROM gitlab_members
    JOIN gitlab_users ON gitlab_users.user_id = gitlab_members.user_id
    ...
    JOIN zuora_account ON zuora_account.account_id = zuora_subscription.account_id
