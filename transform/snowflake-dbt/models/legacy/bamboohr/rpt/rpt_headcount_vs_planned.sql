...
joined AS (
    ...
    SELECT
        ...
        COALESCE(
            TRIM(department_name_changes.new_department_name),
            hire_plan.department
        ) AS department,
        COALESCE(
            headcount.headcount_actual, 0
        ) AS headcount_actual,
        COALESCE(
            headcount.hires_actual, 0
        ) AS hires_actual,
    ...
    LEFT JOIN department_name_changes
              ON
            department_name_changes.old_department_name = hire_plan.department
    LEFT JOIN headcount
              ON headcount.breakout_type = hire_plan.breakout_type
              AND headcount.department = COALESCE(
                department_name_changes.new_department_name,
                hire_plan.department
              )
