...
SELECT 
  ...
  metrics.stage_name,
  metrics.group_name,
  COALESCE(metrics.is_smau,is_smau)                             AS is_smau,
  COALESCE(metrics.is_gmau,is_gmau)                             AS is_gmau,
  metrics.clean_metrics_name,
  metrics.periscope_metrics_name,
  metrics.time_period,
  COALESCE(metrics.is_umau,is_umau)                             AS is_umau,
  COALESCE(metrics.is_paid_gmau,is_paid_gmau)                   AS is_paid_gmau,
