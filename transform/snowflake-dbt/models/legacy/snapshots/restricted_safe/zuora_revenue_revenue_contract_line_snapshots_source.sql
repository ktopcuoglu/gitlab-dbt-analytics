WITH source AS (
  ...
  SELECT *
  FROM {{ source('snapshots', 'zuora_revenue_revenue_contract_line_snapshots') }}

), renamed AS (

/*

  ATTENTION: When a field is added to this snapshot model, add it to the ZUORA_REVENUE_REVENUE_CONTRACT_LINE model to keep the live and snapshot models in alignment.
