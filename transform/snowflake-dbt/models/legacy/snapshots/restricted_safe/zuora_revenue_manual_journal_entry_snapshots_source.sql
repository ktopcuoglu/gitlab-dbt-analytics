 WITH source AS (
  ...
  SELECT *
  FROM {{ source('snapshots', 'zuora_revenue_manual_journal_entry_snapshots') }}

), renamed AS (

/*

  ATTENTION: When a field is added to this snapshot model, add it to the ZUORA_REVENUE_MANUAL_JOURNAL_ENTRY model to keep the live and snapshot models in alignment.
